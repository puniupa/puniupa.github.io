...
showtext_auto()
font_add(
  family = "BIZ-UDGothic",  # 使用する際の名前
  regular = "BIZUDGothic-Regular.ttf"  # フォントファイルのパス
)
ggplot(df_long_promoted, aes(x = factor(測定時点), y = スコア値)) +
  ...
  labs(
       ...
       y = "ココシルスコア") +
   theme(
    text = element_text(family = "BIZ-UDGothic"),
    axis.text = element_text(size = 30),
    axis.title = element_text(size = 40),
    title = element_text(size = 40),
    axis.text.x = element_text(
      margin = margin(t = 0, b = 20),  # 上(top)に20ポイント、下(bottom)に0ポイントのマージン
      size = 30
    ),
    axis.text.y = element_text(
      margin = margin(l=10),  # 上(top)に20ポイント、下(bottom)に0ポイントのマージン
      size = 30
    ),
    axis.title.x = element_text(
      margin = margin(t = 0, b = 10),  # 上(top)に20ポイント、下(bottom)に0ポイントのマージン
      size = 40
    ),
  ...
  scale_x_discrete(labels = c("昇格３ヶ月前", "昇格当月")) +
  annotate("text", 
           x = 1.0,      # x座標（1と2の中間）
           y = 70,  # y座標（データの最大値より少し上）
           label = "p == 0.01", # LaTeX風の数式
           parse = TRUE,  # 数式としてパース
           size = 18,
           family = "BIZ-UDGothic")     # テキストサイズ
# ggsave("昇格者のスコア.png")
```
